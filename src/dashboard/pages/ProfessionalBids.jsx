            ...
            return (
              <div key={b.id} className="card p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-amber-600">{job && categoryLabel(job.category)}</p>
                ...
                <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-slate-100 pt-4 text-sm text-slate-600">
                  ...
                  {job && <Link to={`/pro-dashboard/jobs/${job.id}`} className="ml-auto text-sm font-semibold text-amber-600 hover:underline">Shiko punën</Link>}
                </div>
              </div>
            );
          })}
